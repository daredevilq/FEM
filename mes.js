...
function B(i, j, n) {
    let h = length / n;
    const left = e(i, 0, n) * e(j, 0, n);

    const integralBoundLeft = Math.max(a, (i - 1) * h, (j - 1) * h);
    const integralBoundRight = Math.min((i + 1) * h, (j + 1) * h, b);;

    function fn(x) {
        return de(i, x, n) * de(j, x, n);
    }

    const integral = gaussLegendre(fn, integralBoundLeft, integralBoundRight, 10)
